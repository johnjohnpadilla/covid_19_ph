...
def createTableOrInsert(latitude, longitude):
    global sessionId
    ...
    try:
        ...
        conn = db.cursor()
        table_created = conn.execute('''CREATE TABLE IF NOT EXISTS user_coordinates (sessionIdString, latitude, longitude)''')
        ...
        if result == None:
            # insert in database
            conn.execute("INSERT INTO user_coordinates (sessionIdString, latitude, longitude) VALUES(?, ?, ?)", (sessionIdString, latitude, longitude))
            #updatedConvo = conn.execute("SELECT * FROM conversations WHERE sessionId='" + sessionId + "'")

            db.commit()
            print("Query committed Details:")
            print("sessionIdString:", sessionIdString)
...
"""
    ...
    def _add_browser_connection(self, ws):
        #tornado.web.RequestHandler.saveIpAddress()
        # ip = self.request.headers.get("X-Forwarded-For", self.request.remote_ip)
        # ip = ip.split(',')[-1].strip()
        # print("IP: ", ip)
        # createTableOrInsert(ip)

        # ip = httputil.HT.headers.get("X-Forwarded-For", httputil.HTTPServerRequest.remote_ip)
        # #ip = httputil.HTTPServerRequest.headers
        # ip = ip.split(',')[-1].strip()
        # print("IP: ", ip)
        # createTableOrInsert(ip)
...
class _BrowserWebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        session_cookies = self.request.headers.get('Cookie')
        print("COOKIES: ", session_cookies)

        if 'latitude' in str(session_cookies) and 'longitude' in str(session_cookies):
            lat_start = session_cookies.find('latitude=') + 9
            lat_end = session_cookies.find(';', lat_start)
            latitude = session_cookies[lat_start:lat_end]
            #print("LATITUDE:", latitude)

            long_start = session_cookies.find('longitude=') + 10
            lat_end = session_cookies.find(';', long_start)
            longitude = session_cookies[long_start:lat_end]
            #print("LONGITUDE:", longitude)
            createTableOrInsert(latitude, longitude)
